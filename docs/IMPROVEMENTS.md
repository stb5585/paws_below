...
**Implementation status (August 2026):** Release A's responsive layout, follow-touch/joystick choice, forgiving swept pickups, and platform-appropriate prompts are implemented. Mobile scaling now uses the real dynamic viewport rather than enforcing desktop minimum canvas dimensions. Recorded proximity barks, treasure-direction reveals, a persistent objective, and progress-aware navigation help from Release B are also implemented. Content work now includes a data-driven animal picker, Mochi the bunny, a collect-then-return goal, rabbit foods and pen artwork, per-animal best scores, and dynamic results totals. Asset revisions have been consolidated into canonical files; animal and selected-world atlases load on demand; and the PWA now separates shell/runtime caches, limits HTML fallback to navigation, and presents ready updates explicitly. The environment-rendering overhaul and remaining roadmap work are still future work.
...
- `public/assets/burrow-atlas.webp`
