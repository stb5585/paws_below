...
- Boundary walls now anchor at the shared edge of playable ground instead of the center of an outside grid cell. Open back-wall fences use a full inward anchor so their transparent footprint does not expose a partial outside tile, while internal wall cells retain centered collision placement over complete themed ground foundations. Cozy-light visibility shades opaque floor surfaces and tints environment sprites instead of fading every tile independently, preventing the scene background from reopening divergent seams.
